...
void setup() {
  ...
  EEPROM.update(EEPROM_SCORE, 0);
  EEPROM.update(EEPROM_SCORE + 1, 0);
...
// Pause state
  ...
  if(arduboy.pressed(LEFT_BUTTON) && player.x > 18) {
    ...
    if(arduboy.justPressed(A_BUTTON)) {
        ...
        arduboy.setCursor(60, 1); arduboy.drawLine(player.x + 8, player.y - 1, player.x + 8, 10, WHITE); Rect laserRect = { player.x + 7, player.y - 64, 4, 64 };
    }
  ...
  Rect laserRect = { player.x + 7, player.y - 64, 4, 64 };
...
{
  score = score + 20; sound.tone(NOTE_E4,100, NOTE_C4,70, NOTE_D4,50); enemy1y = -14; enemy1x = random(26,87);
}
}
